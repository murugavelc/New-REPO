...
class Messages extends CI_Controller {
	...
	public function add_message(){
        ...
        if(isset($_POST)) {
            ...
            $name_array = array();
            $config['upload_path'] = './uploads/message/' . $userid . '/';
...
//                $_POST['group_users'][] = $_SESSION['user_id'];
    ...
    public function chat_userList(){
        ...
        if(isset($_POST)) {
            ...
            $data['users'] = $this->Message_model->getAllUsersMe();
            $data['groups'] = $this->Message_model->getAllGroups();
            $res['recent'] = $this->load->view('ajax/chat_recentlist',$data,true);
            $res['users'] = $this->load->view('ajax/chat_userslist',$data,true);
            $res['groups'] = $this->load->view('ajax/chat_groupslist',$data,true);
        }
        echo json_encode($res);
    }
...
//                $_POST['group_users'][] = $_SESSION['user_id'];
    ...
    public function get_messages_by_user(){
        ...
        $group_msg = $this->Message_model->grouped_message($uid);
        if(!empty($group_msg)) {
            foreach ($group_msg as $group) {
                ...
                $this->umessage = $this->Message_model->messagelist($uid,$group['cdate']);
                                    ...
                                    <div class="media-body pad-hor">
                                        <div class="speech">
                                            ...
                                            if($umessage['message'] != ''){
                                                ...
                                                echo '<img  width="140px" src="'.BASE.'uploads/message/'.$send_user->user_id.'/'.$umessage['file'].'">';
                                            }
                                    ...
                                    <div class="media-body pad-hor speech-right">
                                        <div class="speech">
                                            ...
                                            if($umessage['message'] != ''){
                                                ...
                                                echo '<img  width="140px" src="'.BASE.'uploads/message/'.$recive_user->user_id.'/'.$umessage['file'].'">';
                                            }
                                            echo '<p class="speech-time">
                                                <i class="fa fa-clock-o fa-fw"></i>'.date('g:i A',strtotime($umessage['datetime'])).'
                                            </p>
    ...
    public function get_messages_by_group(){
        $gid = $this->input->post('userid');
        $group_msg = $this->Message_model->grouped_message2($gid);
        if(!empty($group_msg)) {
            foreach ($group_msg as $group) {
                $cdate = '';
                if (date('M d, Y', strtotime('now')) == $group['cdate']) {
                ...
                echo '<div class="devider"><div><p>' . $cdate . '</p></div></div>';
                $this->umessage = $this->Message_model->messagelist2($gid,$group['cdate']);
                foreach($this->umessage as $umessage) {
                    $msg_id = $umessage['mid'];
                    ...
                    if($_SESSION['user_id'] == $umessage['sender']) {
                        ...
                        if($umessage['message'] != ''){
                            ...
                            echo '<img  width="140px" src="'.BASE.'uploads/message/group/'.$umessage['group_id'].'/'.$umessage['file'].'">';
                        }
                        echo '<p class="speech-time">
                                                <i class="fa fa-clock-o fa-fw"></i>'.date('g:i A',strtotime($umessage['datetime'])).'
                                            </p>
    ...
    public function searchUsers(){
        ...
        if(isset($_POST['seval'])){
            $data['recent'] = $this->Message_model->searchRes($_POST['seval']);
            $res = $this->load->view('ajax/chat_searchUsers',$data,TRUE);
        }
        echo $res;
    }
